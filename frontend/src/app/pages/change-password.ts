...
@Component({
  ...
  template: `
    <div class="screen screen-pad" style="align-items:center;justify-content:center;background:linear-gradient(160deg,#FFF 0%,#FEF2F2 50%,#F7F8FA 100%)">
...
export class ChangePasswordPage {
  ...
  submit() {
    ...
    this.api.changePassword(this.current(), this.pwNew()).subscribe({
      next: (u) => { this.loading.set(false); this.done.set(true); this.auth.user.set(u); setTimeout(() => this.router.navigateByUrl('/dashboard'), 800); },
